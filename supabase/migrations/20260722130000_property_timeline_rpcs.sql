-- ============================================================
-- 案場歷程 (property timeline) RPCs
-- prototype/case-history-timeline.html 用。
-- 沿用現行儀表板模式：SECURITY DEFINER 原函式對 anon 開放（RBAC dormant 期間），
-- 並比照 Phase 2a 加 api_ wrapper（module: timeline）供未來 cutover。
-- 注意：
--  * 管理費 property_management_fees 只有「最近一次/下次」，非完整歷史（Ragic 補齊前先如實標示）。
--  * property_ownership_history 是死快照，案場切換一律走 property_switch_log。
--  * 修繕只出「報修→完修」一筆區間事件，不倒 maintenance_progress_log（雜訊控制）。
-- ============================================================
...
with ev(d, e, cat, title, "desc", amt) as (
  ...
  -- 業主租金分段（rent_periods 是逐月一筆，gaps-and-islands 壓成「同額連續區段」，顯示每段起迄）
  union all
  ...
  -- 修繕（tickets：報修→完修一筆區間；progress log 不倒入）
  union all
  ...
  -- 請款（payment_request_items 綁案場，彙總到請款單）
  union all
  select pr.application_date, null, 'capex',
         '請款 $'||to_char(x.amt,'FM999,999,999'),
         coalesce('分類：'||x.cats||'，','')||coalesce(nullif(trim(pr.summary),''),'')
           ||coalesce('｜'||pr.applicant_dept,''), x.amt
    from ( select pri.payment_request_id, sum(pri.amount) amt,
                  string_agg(distinct pri.item_category, '、') cats
             from finance.payment_request_items pri
            where pri.property_id = p_property_id
            group by pri.payment_request_id ) x
    join finance.payment_requests pr on pr.id = x.payment_request_id
   where pr.deleted_at is null and coalesce(pr.is_void,false) = false
     and pr.application_date is not null

  -- 帶看（viewing_tasks，經 unit 綁案場；不輸出準租客個資）
  union all
  ...
  -- 簽約任務（signing_tasks；只出狀態與金額，不輸出個資）
  union all
  ...
  -- 水電瓦斯（property_utilities：合約期間＋最近繳費＋下次繳費）
  union all
  ...
  -- 工務日誌（operation_logs：日常作業紀錄，前端預設關閉）
  union all
  ...
  -- LINE 對話萃取事件（property_line_events，LLM 萃取後入庫）
  union all
...
-- ── 權限：比照現行 dormant 期間模式（anon 可用），並附 api_ wrapper ──
grant execute on function public.dashboard_timeline_properties() to anon, authenticated, service_role;
grant execute on function public.dashboard_timeline_events(uuid) to anon, authenticated, service_role;

create or replace function public.api_dashboard_timeline_properties()
returns jsonb language plpgsql security definer set search_path=public as $fn$
begin
  perform public.dashboard_require_any(array[]::text[], array['timeline']);
  return public.dashboard_timeline_properties();
end $fn$;
revoke all on function public.api_dashboard_timeline_properties() from public, anon;
grant execute on function public.api_dashboard_timeline_properties() to authenticated;

create or replace function public.api_dashboard_timeline_events(p_property_id uuid)
returns jsonb language plpgsql security definer set search_path=public as $fn$
begin
  perform public.dashboard_require_any(array[]::text[], array['timeline']);
  return public.dashboard_timeline_events(p_property_id);
end $fn$;
revoke all on function public.api_dashboard_timeline_events(uuid) from public, anon;
grant execute on function public.api_dashboard_timeline_events(uuid) to authenticated;
